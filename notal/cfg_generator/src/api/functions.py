from notal.cfg_generator.src.backend.cfg_generator.cfg_generator import CFGGenerator
from notal.cfg_generator.src.backend.parser.notal_parser import NotalParser
from notal.cfg_generator.src.backend.cfg_generator.ast_parser import *
...
def get_ast_info(subprogram_ast):
    children = subprogram_ast.get_children()
    subprogram_declaration = children[0]
    subprogram_name = subprogram_declaration.get_children()[0].get_notal_src()
    if len(children) == 1:
        return subprogram_name, None
    subprogram_implementation = children[1]
    collected_ast = (
