...
def generate_sendjaja_source_cfgs(dirname, filename):
    dir_path = os.path.join(ROOT_DIR, "datasets", dirname)
    folders = [f for f in os.listdir(dir_path)]
    for folder in folders:
        source_root = os.path.join(dir_path, folder)
        if filename in os.listdir(source_root):
            ...
            cur_cfg = None
            try:
                cur_cfg = generate_cfg_from_file(source_path)
            except Exception as e:
                cur_cfg = None
                # print(e)
            yield nim, cur_cfg
...
def generate_sendjaja_reference_cfgs(dirname):
    dir_path = os.path.join(ROOT_DIR, "datasets", "references", dirname)
    ret = []
    for file in os.listdir(dir_path):
        file_path = os.path.join(dir_path, file)
        ret.append(generate_cfg_from_file(file_path))
    return ret


def do_sendjaja_testing(data_dirs, problem_name):
    EPS = 0.00001
    if isinstance(data_dirs, str):
        data_dirs = [data_dirs]
    data = read_results_data(f"{problem_name}.xlsx")
    ...
    # print(data.columns.tolist())
    unit_time_limit = 5000
    references = generate_sendjaja_reference_cfgs(f"{problem_name}")
    students_score = {}

    for data_dir in data_dirs:
        for nim, cfg in generate_sendjaja_source_cfgs(data_dir, f"{problem_name}.py"):
            if cfg is None:
                students_score[nim] = 0
                continue
            print(f"Grading student {nim}...")
            ...
            for reference in references:
                score, _, _ = compare(collapse(cfg), collapse(reference))
                scores.append(score)
            students_score[nim] = max(scores)

    new_data = []
    for nim, sendjaja_score in students_score.items():
        row = None
        try:
            row = data.loc[int(nim)]
        except:
        ...
        new_data.append({
            ...
            'bb_score': blackbox_score,
            'old_wb_score': kevin_score,
            'new_wb_score': sendjaja_score
        })
        print(f'{kevin_score} {sendjaja_score}')
        assert(abs(kevin_score - sendjaja_score) <= EPS)
    # csv_columns = ['bb_score', 'old_wb_score', 'new_wb_score']
    # random.shuffle(new_data)
    # write_csv(f"{problem_name}.csv", csv_columns, new_data)
...
if __name__ == '__main__':
    ...
    # do_sendjaja_testing(praktikum4_dirs, "empatinteger")
    # do_sendjaja_testing(praktikum6_dirs[-1], "countvokal")
